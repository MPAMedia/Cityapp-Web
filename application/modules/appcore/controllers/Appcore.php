 ...
 * Time: {time}
 */
class Appcore extends MY_Controller
{
    ...
    public function load()
    {
        ...
        //template register
        $method = function () {
            echo $this->getMenusWithView();
        };
      //  $this->template->addView("fetchSidebar", $method);

        //template register
        $method = function () {
            echo $this->getSettingsMenu();
        };
      //  $this->template->addView("fetchSidebarSettings", $method);
    ...
    public function register($context, $options)
    {
        ...
        if (!isset($options['order']) and $options['order'] != 0 and ENVIRONMENT != "itemion") {
        ...
        if (isset(self::$registredModules[$options['order']]) and $options['order'] != 0 and ENVIRONMENT != "itemion") {
    ...
    public function getSettingsMenu()
    {
        ...
        foreach (self::$registredModules as $key => $item) {
