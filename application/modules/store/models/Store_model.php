...
class Store_model extends CI_Model {
    ...
    public function getStores($params=array()){
        ...
        if(isset($category_id) and $category_id>0){
          ...
          if(isset($place_id) and $place_id>0){
            $this->db->where("store.place_id",$place_id);
        }

        if(isset($search) and $search!=""){
            $this->db->like('store.name', $search);
            $this->db->or_like('store.address', $search);
            $this->db->or_like('category.name', $search);
        }
        ...
        foreach ($new_stores_results as $key => $store){
            ...
            if(count($offer)>0){
                ...
                if(isset($content['percent']) and ( $content['percent']>0 OR $content['percent']<0))
                    $new_stores_results[$key]['lastOffer'] = $content['percent']." %";
                else if(isset($content['price']) AND $content['price']>0){
                    $new_stores_results[$key]['lastOffer'] = $this->mOfferModel->parseCurrencyFormat($content['price'],$content['currency']);
                }
    ...
    public function updateStore($params=array()){
         ...
         if(isset($name_ar) AND $name_ar!="" )
        {
            ...
            $errors['name_ar']= Translate::sprint("AR Name empty");
        }
        ...
        if(isset($detail_ar) AND $detail_ar!="" )
        {
            $data["detail_ar"]= htmlentities($detail_ar);
        }
        ...
        if (isset($place_id) AND $place_id > 0) {
            ...
            $errors["place_id"] = Translate::sprint("Place is empty");
        }
...
//
//            }else{
//                $errors['name'] =Messages::STORE_NAME_INVALID;
//            }
...
//                $errors['name'] =Messages::STORE_NAME_INVALID;
//            }
            $data['name_ar'] = Text::input( ucfirst($name_ar));
        }else{
            $errors['name_ar'] = Translate::sprint("AR Name empty");
        }
        ...
        if( isset($phone) && $phone!=""){
            ...
            /* $errors['telephone'] = Messages::STORE_PHONE_EMPTY;*/
            $data['telephone'] = "";
        }
        ...
        if(isset($latitude) and $latitude!=""){
            ...
            if(is_double($latitude)){
                ...
                if($user_id>1)
                    $errors['position'] = Translate::sprint(Messages::USER_LOCATION_ERROR);
            }
        }else{
            if($user_id>1)
                $errors['position'] = Translate::sprint(Messages::USER_LOCATION_ERROR);
        ...
        if(isset($longitude) and $longitude!="" AND $user_id>0){
      ...
      if(isset($detail_ar) and $detail_ar!="" ){
            $data['detail_ar'] = Text::inputWithoutStripTags($detail_ar);
        ...
        if(isset($user_id) AND $user_id>0){
            ...
            if($count==0){
                $errors['user'] = Messages::USER_CREATE_ACCOUNT;
            }else{
        ...
        if(empty($errors) AND !empty($data)){
            ...
            if(count($user_setting)>0) {
                ...
                $nbr_store = $user_setting->nbr_stores;

                if($nbr_store>0 || $nbr_store==-1){


                    if(ENABLE_STORE_AUTO==TRUE || (isset($typeAuth) and $typeAuth=="admin"))
                        $data['status'] = 1;
                    else
                        $data['status'] = 0;
